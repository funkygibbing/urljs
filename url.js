 ...
 * Modified by Ray Walker <hello@raywalker.it>
...
// *** Prototype *** //
    ...
    URL.prototype = {
         ...
         * This takes an array or string and returns the parsed query as an Array of Arrays or value of parameter if found
         *
         * @public
         * @method  query
         * @param   {Array | String}   query   - Optional query to set on the URL
         * @return  {Boolean | URL | String}  the URL query, the URL instance or value of query param (false if not found)
         */
        query : function (query) {
            if (typeof query === 'string') {
                for (var i = 0; i < this._url[QUERY].length; i++) {
                    if (this._url[QUERY][i][0] === query) {
                        return this._url[QUERY][i][1]
                    }
                }
                return false;
            }
        ...
        _set : function (urlPart, val) {
            ...
            if (urlPart === QUERY && val.length) {
                var self = this;
                val.forEach(function (v) {
                    try {
                        // find the query parameter in existing array
                        for (var i = 0; i < self._url[urlPart].length; i++) {
                            if (self._url[urlPart][i][0] === v[0]) {
                                if (undefined !== v[1] && v[1] === false) {
                                    // false but not undefined means unset this parameter
                                    self._url[urlPart].splice(i,1);
                                } else {
                                    // value of undefined is permitted for query string parameters
                                    self._url[urlPart][i] = v;
                                }
                                throw BreakException;
                            }
                        }

                        // query parameter not found so set it
                        self._url[urlPart][i] = v;
                    } catch (e) {
                        if (e !== BreakException) {
                            throw e;
